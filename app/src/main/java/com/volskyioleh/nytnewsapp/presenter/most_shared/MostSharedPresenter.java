package com.volskyioleh.nytnewsapp.presenter.most_shared;
...
public class MostSharedPresenter extends BasePresenterAbs<MostSharedContract.View>
        implements MostSharedContract.Presenter {

    public MostSharedPresenter(ServerCommunicator serverCommunicator) {
    ...
    @Override
    public void getMostSharedArticlesList(){

        getServerCommunicator().buildGetSharedArticlesObservable()
                .subscribe(response -> {
                            insertArticles(response);
                           getView().showList(getAllItems(1));
                        },
    ...
    private void insertArticles(Response response) {
        ...
        for (int i = 0; i < response.results.size(); i++) {
            ...
            articleModels.add(new ArticleModel(article.id, article.url, article.title, article.abstractText, article.media.get(0).mediaMetadata.get(article.media.get(0).mediaMetadata.size() - 1).url, 1, article.byline, getDateMillis(article.published_date), false));
        }
